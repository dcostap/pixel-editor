...
public class CustomTextureRegion {
	...
	public CustomTextureRegion() {
	}

	/** Constructs a region the size of the specified texture. */
	public CustomTextureRegion(Texture texture) {
	...
	public CustomTextureRegion(Texture texture, int width, int height) {
		this.texture = texture;
	...
	/** @param width The width of the texture region. May be negative to flip the sprite when drawn.
	 * @param height The height of the texture region. May be negative to flip the sprite when drawn. */
	public CustomTextureRegion(Texture texture, int x, int y, int width, int height) {
		this.texture = texture;
		setRegion(x, y, width, height);
	}

	public CustomTextureRegion(Texture texture, float u, float v, float u2, float v2) {
		this.texture = texture;
		setRegion(u, v, u2, v2);
	}

	/** Constructs a region with the same texture and coordinates of the specified region. */
	public CustomTextureRegion(CustomTextureRegion region) {
		setRegion(region);
	}
	 ...
	 * @param width The width of the texture region. May be negative to flip the sprite when drawn.
	 * @param height The height of the texture region. May be negative to flip the sprite when drawn. */
	public CustomTextureRegion(CustomTextureRegion region, int x, int y, int width, int height) {
	...
	public void setRegion (CustomTextureRegion region) {
		texture = region.texture;
		setRegion(region.u, region.v, region.u2, region.v2);
	}

	/** Sets the texture to that of the specified region and sets the coordinates relative to the specified region. */
	public void setRegion (CustomTextureRegion region, int x, int y, int width, int height) {
		texture = region.texture;
	...
	public CustomTextureRegion[][] split (int tileWidth, int tileHeight) {
		int x = getRegionX();
		int y = getRegionY();
		...
		int startX = x;
		CustomTextureRegion[][] tiles = new CustomTextureRegion[rows][cols];
		for (int row = 0; row < rows; row++, y += tileHeight) {
			x = startX;
			for (int col = 0; col < cols; col++, x += tileWidth) {
				tiles[row][col] = new CustomTextureRegion(texture, x, y, tileWidth, tileHeight);
			}
		}
	 ...
	 * @param tileHeight a tile's height in pixels
	 * @return a 2D array of TextureRegions indexed by [row][column]. */
	public static CustomTextureRegion[][] split (Texture texture, int tileWidth, int tileHeight) {
		CustomTextureRegion region = new CustomTextureRegion(texture);
